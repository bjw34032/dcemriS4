...
using standard data formats (such as ANALYZE or NIfTI) that may be
visualized and manipulated in \proglang{R} \citep{R} or exported for
accessibility across a wide variety of medical image analysis software
packages.
...
incorporating the \pkg{multicore} package \citep{multicore}.
...
\item The function \code{dcemri.bayes} provides the posterior median
  ...
  with the \pkg{multicore} package by setting the option
  \code{multicore=TRUE}.
...
\pkg{multicore} package (\code{multicore=TRUE}).
...
rm(fit.lm)
@ 
<<RiderNeuroMRI+lm3,echo=FALSE>>=
fit.lm <- list(ktrans=readNIfTI(paste(perf, "ktrans", sep="_")))
x <- 41:220
y <- 21:220
@ 
<<RiderNeuroMRI+lm4,echo=FALSE>>=
...
overlay(dynamic, ifelse(mask, fit.lm$ktrans, NA), w=11, zlim.x=c(32,256), 
        zlim.y=c(0,0.1))
@ 
<<RiderNeuroMRI+lm6,echo=FALSE,results=hide>>=
dev.off()
...
@ 
<<RiderNeuroMRI+map3,echo=FALSE>>=
fit.map <- list(ktrans=readNIfTI(paste(perf, "ktrans", "map", sep="_")))
@ 
<<RiderNeuroMRI+map4,echo=FALSE,results=hide>>=
...
fit.bayes <- dcemri.bayes(conc, acqtimes, mask, model="extended", 
                          aif="fritz.hansen", ab.ktrans=c(log(0.05),1), 
                          ab.kep=c(log(0.7),1), ab.vp=c(1,19)) #, multicore=TRUE)
writeNIfTI(fit.bayes$ktrans, paste(perf, "ktrans", "bayes", sep="_"))
@ 
<<RiderNeuroMRI+bayes2,eval=FALSE,echo=FALSE>>=
...
rm(fit.bayes)
@ 
<<RiderNeuroMRI+bayes3,echo=FALSE,results=hide>>=
...
@ 
<<RiderNeuroMRI+spline2,echo=FALSE,results=hide>>=
...
@ 
<<RiderNeuroMRI+dwi2,echo=FALSE,results=hide>>=
