...
version of R, user name, date, time, etc.  When writing NIfTI-class
objects to disk, the XML-based NIfTI extension is converted into plain
text and saved appropriately.

It should be noted that at this time objects that are written out in
NIfTI format with extensions created by \pkg{decmriS4} will not be
properly read into FSLView
(\url{http://www.fmrib.ox.ac.uk/fsl/fslview/}).  If you prefer to
visualize multidimensional arrays in FSLView, please use the command

<<NIfTI.audit.trail>>=
options(NIfTI.audit.trail=FALSE)
@

to turn off the ``audit trail'' option in \pkg{dcemriS4}.  We are
actively working to better understand how FSLView interprets NIfTI
header extensions with the goal of full integration.  An alternative
viewing platform is available via MRIcroN
...
gradient pulse and $\Delta$ is the time between the two pulses.
